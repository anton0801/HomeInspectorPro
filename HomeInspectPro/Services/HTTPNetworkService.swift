...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(HomeInspectConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: HomeInspectConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://homeinsspectpro.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(HomeInspectConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [22.0, 44.0, 88.0]
